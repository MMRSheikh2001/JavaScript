...
// শুধু In-Stock Product, Price অনুযায়ী কম থেকে বেশি সাজানো, শুধু Title+Price বের করা, টোটাল কত টাকা লাগবে স্টক আউট করতে
//  চাইলে সেটা ক্যালকুলেট করুন।

let inStock = products?.filter((m) => m.inStock === true)?.sort((a, b) => a.price - b.price);

let titlePrice = inStock?.map((m) => `${m.title} ----> ${m.price}`);
console.log(titlePrice);

let totalPrice = inStock.reduce((acc, curr) => {

    return acc += curr.price;

}, 0);

console.log(`Total Money to Stock Out: ${totalPrice} BDT`);